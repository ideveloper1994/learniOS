...
//  Lihnk
//
//  Copyright (c) 2015 Lihnk inc. All rights reserved.
//
...
class RangeSliderThumbLayer: CALayer {
    ...
    override func draw(in ctx: CGContext) {
        if let slider = rangeSlider {
            if let image = slider.thumbImage {
                let thumbFrame = bounds.insetBy(dx: 2.0, dy: 2.0)
                let thumbFrame2 = thumbFrame.offsetBy(dx: -2.0, dy: -2.0)
                ctx.translateBy(x: 0, y: image.size.height);
                ctx.scaleBy(x: 1.0, y: -1.0);
                ctx.draw(image.cgImage!, in: thumbFrame2)
