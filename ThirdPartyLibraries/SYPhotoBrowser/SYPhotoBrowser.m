...
-(void)createCloseButton
{
    ...
    btnClose.frame = CGRectMake(0, 0, 45, 45);
    [btnClose setBackgroundImage:[UIImage imageNamed:@"close_icon_white.png"] forState:UIControlStateNormal];
    [btnClose addTarget:self action:@selector(dismissView) forControlEvents:UIControlEventTouchUpInside];
    [[UIApplication sharedApplication].keyWindow addSubview:btnClose];
    //    [self.scrollView bringSubviewToFront:btnClose];
    [[UIApplication sharedApplication].keyWindow bringSubviewToFront:btnClose];
}
